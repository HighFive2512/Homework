...
for eachticket in uinput:
    ...
    if len(eachticket) == 20:
        ...
        res_first_half, res_second_half = check_if_winner(first_half), check_if_winner(
            second_half
        )
        if "no match" in res_first_half or "no match" in res_second_half:
            ...
            print(
                f'ticket "{eachticket}" - {res_first_half.split(" - ")[1]}{res_first_half.split(" - ")[2]}'
            )
        elif res_first_half.split(" - ")[1] > res_second_half.split(" - ")[1]:
            print(
                f'ticket "{eachticket}" - {res_second_half.split(" - ")[1]}{res_second_half.split(" - ")[2]}'
            )
        else:
            if int(res_first_half.split(" - ")[1]) == 10:
                print(
                    f'ticket "{eachticket}" - {res_first_half.split(" - ")[1]}{res_first_half.split(" - ")[2]} Jackpot!'
                )
            else:
                print(
                    f'ticket "{eachticket}" - {res_first_half.split(" - ")[1]}{res_first_half.split(" - ")[2]}'
