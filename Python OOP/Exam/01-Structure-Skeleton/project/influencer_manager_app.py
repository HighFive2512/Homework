...
class InfluencerManagerApp:
    ...
    def influencer_campaign_report(self, username: str) -> str:
        influencer = next(
            (inf for inf in self.influencers if inf.username == username), None
        )
        if influencer is None:
    ...
    def campaign_statistics(self) -> str:
        ...
        for campaign in sorted(
            self.campaigns, key=lambda c: (len(c.approved_influencers), -c.budget)
        ):
            total_reached_followers = sum(
                influencer.reached_followers(type(campaign).__name__)
